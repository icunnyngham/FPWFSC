...
class PreprocessImage:
    ...
    *in the rotated frame* (found automatically on first use if not
    ...
    def find_conv_center(self, im, ref_im):
        """Refine the stored center by convolving with a reference PSF.

        ``im`` is a frame processed with the *current* center estimate;
        the convolution peak's offset from the frame center is added to
        the stored center.
        """
        conv_im = convolve2d(im, ref_im, mode="same")
